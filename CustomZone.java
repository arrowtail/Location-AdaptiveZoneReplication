...
public class CustomZone {
	static final int ORIGINAL = ZoneSim.ORIGINAL;
	static final int ORIGINAL2 = ZoneSim.ORIGINAL2;
	static final int ORIGINAL3 = ZoneSim.ORIGINAL3;
	static final int ORIGINAL4 = ZoneSim.ORIGINAL4;
...
//						nodes=aNode.getZoneReplication().maintenance(nodes);
	...
	ArrayList<ChordNode> init(ArrayList<ChordNode> nodes) {
		...
		//データのあるノードを決める(他はデータなしノードとして初期化)
		for (i = 0; i < ZoneSim.NODENUM; i++) {
			...
			if (i == ORIGINAL || i == ORIGINAL2 || i== ORIGINAL3 || i==ORIGINAL4) {
				if(i-ZONE<0){
					temp = new ChordNode(i,nodeList.get(i), true, nodeList.get(i+1),nodeList.get(ZoneSim.NODENUM-1),new ZoneReplication(i,false),i+ZONE,ORIGINAL4,i+ZONE+ZONE);
				}else if(i+ZONE > NODENUM-1){
					temp = new ChordNode(i,nodeList.get(i), true, nodeList.get(i+1),nodeList.get(ZoneSim.NODENUM-1),new ZoneReplication(i,false),0,i-ZONE,i-(ZONE+ZONE));
				}else{
					temp = new ChordNode(i,nodeList.get(i), true, nodeList.get(i+1),nodeList.get(ZoneSim.NODENUM-1),new ZoneReplication(i,false),i+ZONE,i-ZONE,i+ZONE+ZONE);
				}
			}else if (i >= ZoneSim.NODENUM-1) {
				temp = new ChordNode(i,nodeList.get(i), false, nodeList.get(0),nodeList.get(i-1),new ZoneReplication(i, false),-1,-1,-1);
			} else {
				temp = new ChordNode(i,nodeList.get(i), false, nodeList.get(i+1),nodeList.get(i-1), new ZoneReplication(i,false),-1,-1,-1);
			}
